...
public class Boss{
    ...
    public void paintMe(Graphics2D g2d) {
        g2d.drawImage(image, xPos - Control.camera.getXOffset()+23, yPos - Control.camera.getYOffset()-36, width, height, null);
    ...
    public void fight(){
        ...
        if(digMove){
            if (wait==0){
                ...
                setMove(new Point(xPos-((wait+1)*((xPos-control.player.getPos().x+118)/30)), yPos));
                wait++;
            } else if (wait == 50){
                if (!(ceilingCheck()) && jumpTop < 1){
                    grounded=false;
            ...
            if (dashBasic) {
                ...
                if (wait <81){
                    if (wait == 0){
                        targetX = control.player.getPos().x-118;
                        startX  = xPos;
                    }
                    jump();
