...
public class Boss{
    ...
    public void attack (Rectangle pPlayer){
        if (wait==0) {
            if (((attackRadius.intersects(pPlayer) && grounded) || isAttacking)&&!busy) {
                isAttacking = true;
                jump();
//                System.out.println("JUMP");
            } else if ((attackCooldown < 75&&attackCooldown!=0)&&!busy) {
                attackCooldown++;
                System.out.println(attackCooldown);
            } else if (attackCooldown >= 75&&!busy) {
                attackCooldown = 0;
                wait++;
            } else if (viewRadius.intersects(pPlayer)&&!busy) {
                if (walkingCooldown<75) {
...
//            //Jump on the position
//            jump();
//        } else if (viewRadius.intersects(pPlayer)){
//            //dash slow, jump
//        } else {
//            //dash fast across arena, don't stop for player. When hitting wall, stun
//            //walk
//            //wait
//        }
    }
