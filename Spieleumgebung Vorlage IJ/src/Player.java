...
public class Player {
    ...
    private int iFrame;
    public boolean wallA = false;
    public boolean floorA = false;
    public boolean ceilingA = false;
    public boolean inTransZone = false;
    ...
    public void setCurrentImage(int pXMove, int pYMove, int pMoveSeq) {
        ...
        if (pXMove == 0 && grounded){
            image = sprites.getSpriteElement(2-direction, 0);
        ...
        if(cooldown>50){
            if (direction == 2){
                image = sprites.getSpriteElement(4, pMoveSeq);
            }else{
                image = sprites.getSpriteElement(5, pMoveSeq);
            }
        }
    }
    ...
    private boolean wallCheck(){
        ...
        for (int i = 0; i < layerList.size(); i++) {
            ...
            Tile temp6;
            try {
                temp = layerList.get(0).tiles[(int) leftTop.getX() / 64][(int) leftTop.getY() / 64];
                temp2 = layerList.get(0).tiles[(int) leftMiddle.getX() / 64][(int) leftMiddle.getY() / 64];
                temp3 = layerList.get(0).tiles[(int) leftBottom.getX() / 64][(int) leftBottom.getY() / 64];
                temp4 = layerList.get(0).tiles[(int) rightTop.getX() / 64][(int) rightTop.getY() / 64];
                temp5 = layerList.get(0).tiles[(int) rightMiddle.getX() / 64][(int) rightMiddle.getY() / 64];
                temp6 = layerList.get(0).tiles[(int) rightBottom.getX() / 64][(int) rightBottom.getY() / 64];
            } catch (Exception e) {
                return true;
            }
            ...
            if (grounded){
                ...
                if (temp.getFlagI().equals("A") || temp2.getFlagI().equals("A") || temp4.getFlagI().equals("A") || temp5.getFlagI().equals("A")) {
                    wallA = true;
                } else {
                    wallA = false;
                }
                ...
                if (temp.getFlagI().equals("A") || temp2.getFlagI().equals("A") || temp3.getFlagI().equals("A") || temp4.getFlagI().equals("A") || temp5.getFlagI().equals("A") || temp6.getFlagI().equals("A")) {
                    wallA = true;
                } else {
                    wallA= false;
                }
    ...
    private boolean floorCheck(){
        ...
        try {
            temp = layerList.get(0).tiles[(int) leftBottom.getX() / 64][(int) leftBottom.getY() / 64];
            temp2 = layerList.get(0).tiles[(int) rightBottom.getX() / 64][(int) rightBottom.getY() / 64];
        } catch (Exception e) {
            return  true;
        }

        if(temp.getFlagI().equals("A")||temp2.getFlagI().equals("A")){
            floorA = true;
        } else {
            floorA = false;
        }
    ...
    private boolean ceilingCheck(){
        ...
        try {
            temp = layerList.get(0).tiles[(int) leftTop.getX() / 64][(int) leftTop.getY() / 64];
            temp2 = layerList.get(0).tiles[(int) rightTop.getX() / 64][(int) rightTop.getY() / 64];
        } catch (Exception e) {
            return  true;
        }
...
//    }
    ...
    public void damageCheck(){
        for (int i = 0; i < control.enemies.size(); i++) {
            ...
            if (control.enemies.get(i).enemyHitBox.intersects(playerHitBox)){
                damage(10);
...
//        renderLifePointBar(g2d);
            playerHitBox = new Rectangle(xPos - Control.camera.getXOffset()+23, yPos - Control.camera.getYOffset(),  18, 44);
...
//              int[] yValues = {xPos - Control.camera.getXOffset()-50, xPos - Control.camera.getXOffset()+50, xPos - Control.camera.getXOffset()};
            ...
            g2d.setColor(Color.green);
            g2d.fillRect(10, 10, hp*5, 50);
            g2d.setColor(Color.red);
            g2d.fillRect(10+(hp*5), 10, (100-hp)*5, 50);
    }
